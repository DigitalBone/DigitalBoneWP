...
/*
*   R3N13R - optimizations to speed up plugin
*
*   1 - Removed multi-site support
*   2 - Removed jetpack support
*   3 - Removed locale switching
*   4 - Mod filter-addon to use our json file (line 208)
*   5 - Added export and import settings support
*   6 - Disable plugin for woocommerce API requests
*
*   See https://celtislab.net/en/wp-plugin-load-filter-addon/
*/
if ( strpos( $_SERVER['REQUEST_URI'], 'wc-api') !== false ) {
    error_log('Plugin Load Filter: WooCommerce API request.');
    exit;
}
...
class Plf_filter {    
    ...
    // MG URL Filter
    static function mg_addon() {
        $json_data = wp_cache_get('mg_plugin_filter', 'mg_plugin_filter');
        if (false === $json_data) {
            $file_path = plugin_dir_path(__FILE__) . '/mg_plugin_filter.json';
            if (file_exists($file_path)) {
                $data = file_get_contents($file_path);
                if ($data !== false) {
                    $json_data = json_decode($data, true);
                    wp_cache_set('mg_plugin_filter', $json_data, 'mg_plugin_filter', 600);
                } else {
                    error_log('MG plugin filter could not be loaded');
                    $json_data = null;
                }
            } else {
                error_log('MG plugin filter not found at ' . plugin_dir_path(__FILE__));
                $json_data = null;
            }
            error_log('Loaded: mg_plugin_filter.json');
        }
        return $json_data;
    }
     ...
     * @param $option   : option data eg. 'active_plugins', 'active_sitewide_plugins', 'celtispack_active_modules' ...
    ...
    static function plf_filter( $option, $default = false) {
        ...
        // R3N13R - Make sure $act_plugins is unique
        $act_plugins = array_unique($act_plugins);
                                        ...
                                        if(!empty($single_opt) && $single_opt['filter'] === 'include'){
                                            ...
                                            if(!empty($single_opt[$devtype])){
                                                if(false !== strpos($single_opt[$devtype], $p_key)){
                                                    ...
                                                    if(strpos($p_key, 'celtispack/') !== false && strpos($single_opt[$devtype], 'celtispack_module/') !== false)
                                                        $unload = false;
                                                }                                                
                                            }                                            
                                        }
                                    ...
                                    if($pgfopt === false){
                                        if(!empty($type) && !empty($filter['group'][$type]['plugins'])){
                                            if(in_array($p_key, array_map("trim", explode(',', $filter['group'][$type]['plugins'])))){
                                                ...
                                                if(strpos($p_key, 'celtispack/') !== false && strpos($filter['group'][$type]['plugins'], 'celtispack_module/') !== false)
                                                    $unload = false;
                                            }
                                        }
                                    }
        ...
        // R3N13R - Make sure $new_plugins is unique
        $new_plugins = array_unique($new_plugins);
